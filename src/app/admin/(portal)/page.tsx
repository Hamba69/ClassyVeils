    ...
    <div className="space-y-12 sm:space-y-16">
      ...
      {(categories as Category[] | null)?.map((category) => (
        <section key={category.slug}>
          ...
          <div className="mt-4 divide-y divide-line overflow-hidden rounded-2xl border border-line bg-white/35">
            ...
            {byCategory(category.slug).map((veil, i, arr) => (
              <div key={veil.id} className="grid grid-cols-[3.5rem_1fr] gap-3 p-4 sm:flex sm:items-center sm:gap-4">
                ...
                <div className="col-span-2 flex flex-wrap items-center gap-2 text-ink/50 sm:col-span-1">
                  <form action={moveVeil.bind(null, veil.id, category.slug, "up")}>
                    <button
                      type="submit"
                      disabled={i === 0}
                      className="grid min-h-11 min-w-11 place-items-center rounded-full border border-line hover:bg-line disabled:opacity-30"
                      aria-label="Move up"
                    >
                      ↑
                    </button>
                ...
                <Link
                  ...
                  className="inline-flex min-h-11 items-center rounded-full border border-line px-4 text-xs text-ink/70 hover:border-plum hover:text-plum"
                >
                  Edit
                </Link>
          ...
          <details className="mt-4 rounded-2xl border border-line bg-white/35 p-4">
            ...
            <form action={createVeil} className="mt-4 space-y-3">
              ...
              <label className="flex min-h-11 items-center gap-3 rounded-xl border border-line bg-white px-3 text-sm text-ink/70">
                <input type="checkbox" name="is_featured" className="h-5 w-5" />
                Feature on homepage
              </label>
              <ImageUploadPreview name="photos" label="Product photos" />
              <div className="sticky bottom-3 z-10 rounded-2xl border border-line bg-linen/95 p-2 shadow-lg backdrop-blur">
                <button type="submit" className="min-h-12 w-full rounded-xl bg-ink px-4 text-sm font-medium text-linen hover:bg-plum">Add veil</button>
              </div>
            </form>
          </details>
        </section>
